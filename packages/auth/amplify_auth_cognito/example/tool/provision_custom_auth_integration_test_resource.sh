...
deploy_lambda_stack () {
    ...
    aws cloudformation deploy --profile=$profileName --template-file ./tool/${1}Lambda/cloudformation.json --stack-name ${1}Stack --parameter-overrides deploymentBucketName=$deploymentBucket s3Key=$2 env=test userpoolId=$userpoolId stackId=$stackId userpoolArn=$userpoolArn --capabilities CAPABILITY_NAMED_IAM
}
...
deploy_lambda_stack "AdminCreateUser" $s3AdminCreateUserKey
deploy_lambda_stack "DeleteUser" $s3DeleteUserKey
deploy_lambda_stack $create $s3CreateAuthChallengeKey
deploy_lambda_stack $define $s3DefineAuthChallengeKey
deploy_lambda_stack $verify $s3VerifyAuthChallengeKey
...
createChallengeArn=$(aws lambda get-function --function-name CreateAuthChallenge_CustomIntegTest --query Configuration.FunctionArn)
defineChallengeArn=$(aws lambda get-function --function-name DefineAuthChallenge_CustomIntegTest --query Configuration.FunctionArn)
verifyChallengeArn=$(aws lambda get-function --function-name VerifyAuthChallenge_CustomIntegTest --query Configuration.FunctionArn)

echo "Updated Trigger permissions..."
aws lambda add-permission --function-name CreateAuthChallenge_CustomIntegTest --statement-id create-auth-challenge-invoke-permission --action lambda:InvokeFunction --principal cognito-idp.amazonaws.com --source-arn $userpoolArn
aws lambda add-permission --function-name VerifyAuthChallenge_CustomIntegTest --statement-id verify-auth-challenge-invoke-permission --action lambda:InvokeFunction --principal cognito-idp.amazonaws.com --source-arn $userpoolArn
aws lambda add-permission --function-name DefineAuthChallenge_CustomIntegTest --statement-id define-auth-challenge-invoke-permission --action lambda:InvokeFunction --principal cognito-idp.amazonaws.com --source-arn $userpoolArn
