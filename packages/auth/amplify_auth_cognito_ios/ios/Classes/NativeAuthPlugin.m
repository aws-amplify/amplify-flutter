...
+ (NativeAuthSession *)fromMap:(NSDictionary *)dict {
  ...
  pigeonResult.userPoolTokens = [NativeUserPoolTokens fromMap:GetNullableObject(dict, @"userPoolTokens")];
  pigeonResult.identityId = GetNullableObject(dict, @"identityId");
  pigeonResult.awsCredentials = [NativeAWSCredentials fromMap:GetNullableObject(dict, @"awsCredentials")];
  return pigeonResult;
}
- (NSDictionary *)toMap {
  return @{
...
void NativeAuthBridgeSetup(id<FlutterBinaryMessenger> binaryMessenger, NSObject<NativeAuthBridge> *api) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:@"dev.flutter.pigeon.NativeAuthBridge.signIn"
        binaryMessenger:binaryMessenger
        codec:NativeAuthBridgeGetCodec()        ];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(signInUrl:callbackUrlScheme:preferPrivateSession:browserPackageName:completion:)], @"NativeAuthBridge api (%@) doesn't respond to @selector(signInUrl:callbackUrlScheme:preferPrivateSession:browserPackageName:completion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        ...
        [api signInUrl:arg_url callbackUrlScheme:arg_callbackUrlScheme preferPrivateSession:arg_preferPrivateSession browserPackageName:arg_browserPackageName completion:^(NSDictionary<NSString *, NSString *> *_Nullable output, FlutterError *_Nullable error) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:@"dev.flutter.pigeon.NativeAuthBridge.signOut"
        binaryMessenger:binaryMessenger
        codec:NativeAuthBridgeGetCodec()        ];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(signOutUrl:callbackUrlScheme:browserPackageName:completion:)], @"NativeAuthBridge api (%@) doesn't respond to @selector(signOutUrl:callbackUrlScheme:browserPackageName:completion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        NSString *arg_url = GetNullableObjectAtIndex(args, 0);
        NSString *arg_callbackUrlScheme = GetNullableObjectAtIndex(args, 1);
        NSString *arg_browserPackageName = GetNullableObjectAtIndex(args, 2);
        [api signOutUrl:arg_url callbackUrlScheme:arg_callbackUrlScheme browserPackageName:arg_browserPackageName completion:^(FlutterError *_Nullable error) {
          callback(wrapResult(nil, error));
        }];
      }];
    }
