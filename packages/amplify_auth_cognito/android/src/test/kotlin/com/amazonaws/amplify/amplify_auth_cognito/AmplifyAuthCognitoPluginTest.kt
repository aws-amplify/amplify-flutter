...
@RunWith(RobolectricTestRunner::class) class AmplifyAuthCognitoPluginTest {

    lateinit var plugin: AuthCognito

    private val mockResult = mock(MethodChannel.Result::class.java)
    private val codeDeliveryDetails =
        AuthCodeDeliveryDetails(
            "test@test.com", AuthCodeDeliveryDetails.DeliveryMedium.EMAIL,
            "email"
        )
    private val signUpStep =
        AuthNextSignUpStep(AuthSignUpStep.CONFIRM_SIGN_UP_STEP, emptyMap(), codeDeliveryDetails)
    private val signInStep =
        AuthNextSignInStep(
            AuthSignInStep.CONFIRM_SIGN_IN_WITH_SMS_MFA_CODE, emptyMap(),
            codeDeliveryDetails
        )
    private val resetStep =
        AuthNextResetPasswordStep(
            AuthResetPasswordStep.CONFIRM_RESET_PASSWORD_WITH_CODE,
            emptyMap(), codeDeliveryDetails
        )
    ...
    @Test fun signUp_returnsSuccess() {
        // Arrange
        doAnswer { invocation: InvocationOnMock ->
            plugin.prepareSignUpResult(mockResult, mockSignUpResult)
            null as Void?
        }.`when`(mockAuth).signUp(
            anyString(), anyString(), any(AuthSignUpOptions::class.java),
        ...
        val data: HashMap<*, *> = hashMapOf(
            "username" to "testUser", "password" to "testPassword",
            "options" to options
        )
    ...
    @Test fun confirmSignUp_returnsSuccess() {
        // Arrange
        doAnswer { invocation: InvocationOnMock ->
            plugin.prepareSignUpResult(mockResult, mockSignUpResult)
            null as Void?
        ...
        val data = hashMapOf("username" to "testUser", "confirmationCode" to "confirmationCode")
        val arguments = hashMapOf("data" to data)
    ...
    @Test fun resendSignUpCode_returnsSuccess() {
        // Arrange
        doAnswer { invocation: InvocationOnMock ->
            plugin.prepareSignUpResult(mockResult, mockSignUpResult)
            null as Void?
            ...
            .resendSignUpCode(
                anyString(), ArgumentMatchers.any<Consumer<AuthSignUpResult>>(),
                ArgumentMatchers.any<Consumer<AuthException>>()
            )

        val data: HashMap<*, *> = hashMapOf("username" to "testUser")
    ...
    @Test fun confirmSignIn_returnsSuccess() {
        // Arrange
        doAnswer { invocation: InvocationOnMock ->
            ...
            .confirmSignIn(
                anyString(), ArgumentMatchers.any<Consumer<AuthSignInResult>>(),
                ArgumentMatchers.any<Consumer<AuthException>>()
            )

        val data: HashMap<*, *> = hashMapOf("confirmationCode" to "confirmationCode")
        val arguments: HashMap<String, Any> = hashMapOf("data" to data)
    ...
    @Test fun signOut_returnsSuccess() {
        // Arrange
        doAnswer { invocation: InvocationOnMock ->
    ...
    @Test fun updatePassword_returnsSuccess() {
        // Arrange
        doAnswer { invocation: InvocationOnMock ->
            plugin.prepareUpdatePasswordResult(mockResult)
            null as Void?
        }.`when`(mockAuth).updatePassword(
            anyString(), anyString(), ArgumentMatchers.any<Action>(),
            ArgumentMatchers.any<Consumer<AuthException>>()
        )

        val data: HashMap<*, *> =
            hashMapOf("newPassword" to "newPassword", "oldPassword" to "oldPassword")
        val arguments: HashMap<String, Any> = hashMapOf("data" to data)
    ...
    @Test fun resetPassword_returnsSuccess() {
        // Arrange
        doAnswer { invocation: InvocationOnMock ->
        ...
        val data: HashMap<*, *> = hashMapOf("username" to "username")
        val arguments: HashMap<String, Any> = hashMapOf("data" to data)
    ...
    @Test fun confirmPassword_returnsSuccess() {
        // Arrange
        doAnswer { invocation: InvocationOnMock ->
            plugin.prepareUpdatePasswordResult(mockResult)
            null as Void?
        ...
        val data: HashMap<*, *> =
            hashMapOf(
                "username" to "username", "newPassword" to "newPassword",
    ...
    @Test fun getCurrentUser_returnsSuccess() {
        // Arrange
        doAnswer { invocation: InvocationOnMock ->
            plugin.prepareUpdatePasswordResult(mockResult)
            null as Void?
        ...
        val data: HashMap<*, *> = hashMapOf("username" to "username", "userSub" to "userSub")
        val arguments: HashMap<String, Any> = hashMapOf("data" to data)
    ...
    @Test fun fetchUserAttributes_returnsSuccess() {
        // Arrange
        ...
        doAnswer { invocation: InvocationOnMock ->
            ...
            null as Void?
        }.`when`(mockAuth).fetchUserAttributes(
            ArgumentMatchers.any<Consumer<MutableList<AuthUserAttribute>>>(),
            ArgumentMatchers.any<Consumer<AuthException>>()
        )
        ...
        var mockedResponse: List<Map<String, String>> =
            listOf(mapOf("key" to "email", "value" to "test@test.com"))
    ...
    @Test fun fetchSession_returnsSuccess() {
        ...
        val data: HashMap<*, *> = hashMapOf("getAWSCredentials" to true)
        val arguments = hashMapOf("data" to data)
        val call = MethodCall("fetchAuthSession", arguments)
        val res = mapOf(
            "isSignedIn" to true, "identityId" to "id", "userSub" to "sub",
            "credentials" to mapOf("accessKey" to "access", "secretKey" to "secret"),
            "tokens" to mapOf(
                "accessToken" to "access", "idToken" to "id",
    ...
    @Test fun signInWithWebUI_returnsSuccess() {
        // Arrange
        doAnswer { invocation: InvocationOnMock ->
            plugin.prepareSignInResult(mockResult, mockSignInResult)
            null as Void?
            ...
            .signInWithWebUI(
                any(), ArgumentMatchers.any<Consumer<AuthSignInResult>>(),
                ArgumentMatchers.any<Consumer<AuthException>>()
            )
        val data: HashMap<*, *> = HashMap<String, String>()
        val arguments: HashMap<String, Any?> = hashMapOf("data" to data)
    ...
    @Test fun signInWithSocialWebUI_returnsSuccess() {
        // Arrange
        doAnswer { invocation: InvocationOnMock ->
            plugin.prepareSignInResult(mockResult, mockSignInResult)
            null as Void?
        }.`when`(mockAuth).signInWithSocialWebUI(
            any(), ArgumentMatchers.any<Activity>(),
            ArgumentMatchers.any<Consumer<AuthSignInResult>>(),
            ArgumentMatchers.any<Consumer<AuthException>>()
        )

        val data: HashMap<*, *> = hashMapOf("authProvider" to "google")
        val arguments: HashMap<String, Any> = hashMapOf("data" to data)
        val call = MethodCall("signInWithWebUI", arguments)
