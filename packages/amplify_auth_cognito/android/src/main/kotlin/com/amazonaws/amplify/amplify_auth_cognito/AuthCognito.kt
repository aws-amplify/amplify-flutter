...
public class AuthCognito :
    ...
    @VisibleForTesting constructor(
        hubEventHandler: AuthCognitoHubEventStreamHandler,
        activity: Activity,
    ) {
        authCognitoHubEventStreamHandler = AuthCognitoHubEventStreamHandler()
    ...
    override fun onAttachedToEngine(
        ...
        val executor = flutterPluginBinding.getFlutterEngine().getDartExecutor()
        channel = MethodChannel(executor, "com.amazonaws.amplify/auth_cognito")
        ...
        try {
            ...
            LOG.error(
                "Failed to add AuthCognito plugin. Is Amplify already configured and app restarted?"
            )
    ...
    private fun onConfirmSignUp(
        @NonNull flutterResult: Result,
        @NonNull request: HashMap<String, *>,
    ) {
        try {
    ...
    private fun onResendSignUpCode(
        @NonNull flutterResult: Result,
        @NonNull request: HashMap<String, *>,
    ) {
        try {
    ...
    private fun onConfirmPassword(
        ...
        try {
            ...
            Amplify.Auth.confirmResetPassword(
                req.newPassword, req.confirmationCode,
                { -> prepareUpdatePasswordResult(flutterResult) },
                { error -> errorHandler.handleAuthError(flutterResult, error) }
            )
        } catch (e: Exception) {
    ...
    private fun onFetchAuthSession(
        @NonNull flutterResult: Result,
        @NonNull request: HashMap<String, *>,
    ) {
                    ...
                    if (req.getAWSCredentials) {
                        ...
                        when (cognitoAuthSession.identityId.type) {
                            ...
                            AuthSessionResult.Type.FAILURE -> {
                                // checking case where user pool data is available
                                // but id pool does not exist
                                if (result.identityId.error is
                                    AuthException.InvalidAccountTypeException &&
                                    result.awsCredentials.error is
                                    AuthException.InvalidAccountTypeException &&
        ...
        try {
            ...
            if (user is AuthUser) {
                ...
                throw AuthException.SignedOutException(
                    AuthException.GuestAccess.GUEST_ACCESS_DISABLED
                )
            }
            ...
            if (req.provider == null) {
                ...
                mainActivity.let {
                    Amplify.Auth.signInWithSocialWebUI(
                        AuthProvider::class.java.getMethod(req.provider!!)
                            .invoke(null) as AuthProvider,
                        it,
                        { result ->
                            if (!resultSubmitted) {
                                resultSubmitted = true
    ...
    fun prepareResendSignUpCodeResult(
        @NonNull flutterResult: Result,
        @NonNull result: AuthSignUpResult,
    ) {
    ...
    fun prepareResetPasswordResult(
        @NonNull flutterResult: Result,
        @NonNull result: AuthResetPasswordResult,
    ) {
    ...
    fun prepareCognitoSessionResult(
        @NonNull flutterResult: Result,
        @NonNull result: AWSCognitoAuthSession,
    ) {
    ...
    fun prepareFetchAttributesResult(
        @NonNull flutterResult: Result,
        @NonNull attributes: List<AuthUserAttribute>,
    ) {
    ...
    fun prepareCognitoSessionFailure(
        @NonNull flutterResult: Result,
        @NonNull result: AWSCognitoAuthSession,
    ) {
