...
class EnumGenerator extends LibraryGenerator<EnumShape> {
  ...
  late final List<MemberShape> sortedDefinitions =
      shape.enumValues.toList()..sort((a, b) {
        return a.enumVariantName.compareTo(b.enumVariantName);
      });

  late final List<EnumValueTrait> sortedEnumValues =
      sortedDefinitions
          .map((def) => def.expectTrait<EnumValueTrait>())
          .toList();

  Reference get valueType =>
      shape.getType() == ShapeType.intEnum
          ? DartTypes.core.int
          ...
          ..assignment =
              literalList(
                sortedDefinitions.map(
                  (e) => symbol.property(e.enumVariantName),
                ),
                symbol,
              ).code,
  );
