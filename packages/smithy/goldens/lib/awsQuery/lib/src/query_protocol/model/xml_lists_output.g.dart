// GENERATED CODE - DO NOT MODIFY BY HAND

part of aws_query_v1.query_protocol.model.xml_lists_output;

// **************************************************************************
// BuiltValueGenerator
// **************************************************************************

class _$XmlListsOutput extends XmlListsOutput {
  @override
  final _i4.BuiltList<String>? stringList;
  @override
  final _i4.BuiltSet<String>? stringSet;
  @override
  final _i4.BuiltList<int>? integerList;
  @override
  final _i4.BuiltList<bool>? booleanList;
  @override
  final _i4.BuiltList<DateTime>? timestampList;
  @override
  final _i4.BuiltList<_i2.FooEnum>? enumList;
  @override
  final _i4.BuiltList<int>? intEnumList;
  @override
  final _i4.BuiltList<_i4.BuiltList<String>>? nestedStringList;
  @override
  final _i4.BuiltList<String>? renamedListMembers;
  @override
  final _i4.BuiltList<String>? flattenedList;
  @override
  final _i4.BuiltList<String>? flattenedList2;
  @override
  final _i4.BuiltList<String>? flattenedListWithMemberNamespace;
  @override
  final _i4.BuiltList<String>? flattenedListWithNamespace;
  @override
  final _i4.BuiltList<_i3.StructureListMember>? structureList;

  factory _$XmlListsOutput([void Function(XmlListsOutputBuilder)? updates]) =>
      (new XmlListsOutputBuilder()..update(updates))._build();

  _$XmlListsOutput._(
      {this.stringList,
      this.stringSet,
      this.integerList,
      this.booleanList,
      this.timestampList,
      this.enumList,
      this.intEnumList,
      this.nestedStringList,
      this.renamedListMembers,
      this.flattenedList,
      this.flattenedList2,
      this.flattenedListWithMemberNamespace,
      this.flattenedListWithNamespace,
      this.structureList})
      : super._();

  @override
  XmlListsOutput rebuild(void Function(XmlListsOutputBuilder) updates) =>
      (toBuilder()..update(updates)).build();

  @override
  XmlListsOutputBuilder toBuilder() =>
      new XmlListsOutputBuilder()..replace(this);

  @override
  bool operator ==(Object other) {
    if (identical(other, this)) return true;
    return other is XmlListsOutput &&
        stringList == other.stringList &&
        stringSet == other.stringSet &&
        integerList == other.integerList &&
        booleanList == other.booleanList &&
        timestampList == other.timestampList &&
        enumList == other.enumList &&
        intEnumList == other.intEnumList &&
        nestedStringList == other.nestedStringList &&
        renamedListMembers == other.renamedListMembers &&
        flattenedList == other.flattenedList &&
        flattenedList2 == other.flattenedList2 &&
        flattenedListWithMemberNamespace ==
            other.flattenedListWithMemberNamespace &&
        flattenedListWithNamespace == other.flattenedListWithNamespace &&
        structureList == other.structureList;
  }

  @override
  int get hashCode {
    var _$hash = 0;
    _$hash = $jc(_$hash, stringList.hashCode);
    _$hash = $jc(_$hash, stringSet.hashCode);
    _$hash = $jc(_$hash, integerList.hashCode);
    _$hash = $jc(_$hash, booleanList.hashCode);
    _$hash = $jc(_$hash, timestampList.hashCode);
    _$hash = $jc(_$hash, enumList.hashCode);
    _$hash = $jc(_$hash, intEnumList.hashCode);
    _$hash = $jc(_$hash, nestedStringList.hashCode);
    _$hash = $jc(_$hash, renamedListMembers.hashCode);
    _$hash = $jc(_$hash, flattenedList.hashCode);
    _$hash = $jc(_$hash, flattenedList2.hashCode);
    _$hash = $jc(_$hash, flattenedListWithMemberNamespace.hashCode);
    _$hash = $jc(_$hash, flattenedListWithNamespace.hashCode);
    _$hash = $jc(_$hash, structureList.hashCode);
    _$hash = $jf(_$hash);
    return _$hash;
  }
}

class XmlListsOutputBuilder
    implements Builder<XmlListsOutput, XmlListsOutputBuilder> {
  _$XmlListsOutput? _$v;

  _i4.ListBuilder<String>? _stringList;
  _i4.ListBuilder<String> get stringList =>
      _$this._stringList ??= new _i4.ListBuilder<String>();
  set stringList(_i4.ListBuilder<String>? stringList) =>
      _$this._stringList = stringList;

  _i4.SetBuilder<String>? _stringSet;
  _i4.SetBuilder<String> get stringSet =>
      _$this._stringSet ??= new _i4.SetBuilder<String>();
  set stringSet(_i4.SetBuilder<String>? stringSet) =>
      _$this._stringSet = stringSet;

  _i4.ListBuilder<int>? _integerList;
  _i4.ListBuilder<int> get integerList =>
      _$this._integerList ??= new _i4.ListBuilder<int>();
  set integerList(_i4.ListBuilder<int>? integerList) =>
      _$this._integerList = integerList;

  _i4.ListBuilder<bool>? _booleanList;
  _i4.ListBuilder<bool> get booleanList =>
      _$this._booleanList ??= new _i4.ListBuilder<bool>();
  set booleanList(_i4.ListBuilder<bool>? booleanList) =>
      _$this._booleanList = booleanList;

  _i4.ListBuilder<DateTime>? _timestampList;
  _i4.ListBuilder<DateTime> get timestampList =>
      _$this._timestampList ??= new _i4.ListBuilder<DateTime>();
  set timestampList(_i4.ListBuilder<DateTime>? timestampList) =>
      _$this._timestampList = timestampList;

  _i4.ListBuilder<_i2.FooEnum>? _enumList;
  _i4.ListBuilder<_i2.FooEnum> get enumList =>
      _$this._enumList ??= new _i4.ListBuilder<_i2.FooEnum>();
  set enumList(_i4.ListBuilder<_i2.FooEnum>? enumList) =>
      _$this._enumList = enumList;

  _i4.ListBuilder<int>? _intEnumList;
  _i4.ListBuilder<int> get intEnumList =>
      _$this._intEnumList ??= new _i4.ListBuilder<int>();
  set intEnumList(_i4.ListBuilder<int>? intEnumList) =>
      _$this._intEnumList = intEnumList;

  _i4.ListBuilder<_i4.BuiltList<String>>? _nestedStringList;
  _i4.ListBuilder<_i4.BuiltList<String>> get nestedStringList =>
      _$this._nestedStringList ??= new _i4.ListBuilder<_i4.BuiltList<String>>();
  set nestedStringList(
          _i4.ListBuilder<_i4.BuiltList<String>>? nestedStringList) =>
      _$this._nestedStringList = nestedStringList;

  _i4.ListBuilder<String>? _renamedListMembers;
  _i4.ListBuilder<String> get renamedListMembers =>
      _$this._renamedListMembers ??= new _i4.ListBuilder<String>();
  set renamedListMembers(_i4.ListBuilder<String>? renamedListMembers) =>
      _$this._renamedListMembers = renamedListMembers;

  _i4.ListBuilder<String>? _flattenedList;
  _i4.ListBuilder<String> get flattenedList =>
      _$this._flattenedList ??= new _i4.ListBuilder<String>();
  set flattenedList(_i4.ListBuilder<String>? flattenedList) =>
      _$this._flattenedList = flattenedList;

  _i4.ListBuilder<String>? _flattenedList2;
  _i4.ListBuilder<String> get flattenedList2 =>
      _$this._flattenedList2 ??= new _i4.ListBuilder<String>();
  set flattenedList2(_i4.ListBuilder<String>? flattenedList2) =>
      _$this._flattenedList2 = flattenedList2;

  _i4.ListBuilder<String>? _flattenedListWithMemberNamespace;
  _i4.ListBuilder<String> get flattenedListWithMemberNamespace =>
      _$this._flattenedListWithMemberNamespace ??=
          new _i4.ListBuilder<String>();
  set flattenedListWithMemberNamespace(
          _i4.ListBuilder<String>? flattenedListWithMemberNamespace) =>
      _$this._flattenedListWithMemberNamespace =
          flattenedListWithMemberNamespace;

  _i4.ListBuilder<String>? _flattenedListWithNamespace;
  _i4.ListBuilder<String> get flattenedListWithNamespace =>
      _$this._flattenedListWithNamespace ??= new _i4.ListBuilder<String>();
  set flattenedListWithNamespace(
          _i4.ListBuilder<String>? flattenedListWithNamespace) =>
      _$this._flattenedListWithNamespace = flattenedListWithNamespace;

  _i4.ListBuilder<_i3.StructureListMember>? _structureList;
  _i4.ListBuilder<_i3.StructureListMember> get structureList =>
      _$this._structureList ??= new _i4.ListBuilder<_i3.StructureListMember>();
  set structureList(_i4.ListBuilder<_i3.StructureListMember>? structureList) =>
      _$this._structureList = structureList;

  XmlListsOutputBuilder() {
    XmlListsOutput._init(this);
  }

  XmlListsOutputBuilder get _$this {
    final $v = _$v;
    if ($v != null) {
      _stringList = $v.stringList?.toBuilder();
      _stringSet = $v.stringSet?.toBuilder();
      _integerList = $v.integerList?.toBuilder();
      _booleanList = $v.booleanList?.toBuilder();
      _timestampList = $v.timestampList?.toBuilder();
      _enumList = $v.enumList?.toBuilder();
      _intEnumList = $v.intEnumList?.toBuilder();
      _nestedStringList = $v.nestedStringList?.toBuilder();
      _renamedListMembers = $v.renamedListMembers?.toBuilder();
      _flattenedList = $v.flattenedList?.toBuilder();
      _flattenedList2 = $v.flattenedList2?.toBuilder();
      _flattenedListWithMemberNamespace =
          $v.flattenedListWithMemberNamespace?.toBuilder();
      _flattenedListWithNamespace = $v.flattenedListWithNamespace?.toBuilder();
      _structureList = $v.structureList?.toBuilder();
      _$v = null;
    }
    return this;
  }

  @override
  void replace(XmlListsOutput other) {
    ArgumentError.checkNotNull(other, 'other');
    _$v = other as _$XmlListsOutput;
  }

  @override
  void update(void Function(XmlListsOutputBuilder)? updates) {
    if (updates != null) updates(this);
  }

  @override
  XmlListsOutput build() => _build();

  _$XmlListsOutput _build() {
    _$XmlListsOutput _$result;
    try {
      _$result = _$v ??
          new _$XmlListsOutput._(
              stringList: _stringList?.build(),
              stringSet: _stringSet?.build(),
              integerList: _integerList?.build(),
              booleanList: _booleanList?.build(),
              timestampList: _timestampList?.build(),
              enumList: _enumList?.build(),
              intEnumList: _intEnumList?.build(),
              nestedStringList: _nestedStringList?.build(),
              renamedListMembers: _renamedListMembers?.build(),
              flattenedList: _flattenedList?.build(),
              flattenedList2: _flattenedList2?.build(),
              flattenedListWithMemberNamespace:
                  _flattenedListWithMemberNamespace?.build(),
              flattenedListWithNamespace: _flattenedListWithNamespace?.build(),
              structureList: _structureList?.build());
    } catch (_) {
      late String _$failedField;
      try {
        _$failedField = 'stringList';
        _stringList?.build();
        _$failedField = 'stringSet';
        _stringSet?.build();
        _$failedField = 'integerList';
        _integerList?.build();
        _$failedField = 'booleanList';
        _booleanList?.build();
        _$failedField = 'timestampList';
        _timestampList?.build();
        _$failedField = 'enumList';
        _enumList?.build();
        _$failedField = 'intEnumList';
        _intEnumList?.build();
        _$failedField = 'nestedStringList';
        _nestedStringList?.build();
        _$failedField = 'renamedListMembers';
        _renamedListMembers?.build();
        _$failedField = 'flattenedList';
        _flattenedList?.build();
        _$failedField = 'flattenedList2';
        _flattenedList2?.build();
        _$failedField = 'flattenedListWithMemberNamespace';
        _flattenedListWithMemberNamespace?.build();
        _$failedField = 'flattenedListWithNamespace';
        _flattenedListWithNamespace?.build();
        _$failedField = 'structureList';
        _structureList?.build();
      } catch (e) {
        throw new BuiltValueNestedFieldError(
            r'XmlListsOutput', _$failedField, e.toString());
      }
      rethrow;
    }
    replace(_$result);
    return _$result;
  }
}

// ignore_for_file: deprecated_member_use_from_same_package,type=lint
