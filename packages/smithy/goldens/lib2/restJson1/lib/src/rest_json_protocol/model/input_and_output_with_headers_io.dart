...
abstract class InputAndOutputWithHeadersIo
  ...
  factory InputAndOutputWithHeadersIo.fromRequest(
    ...
    if (request.headers['X-StringSet'] != null) {
      b.headerStringSet.addAll(
        _i1.parseHeader(request.headers['X-StringSet']!).map((el) => el.trim()),
      );
    }
    if (request.headers['X-IntegerList'] != null) {
      b.headerIntegerList.addAll(
        _i1
    ...
    if (request.headers['X-IntegerEnum'] != null) {
      b.headerIntegerEnum = IntegerEnum.values.byValue(
        int.parse(request.headers['X-IntegerEnum']!),
      );
    }
    if (request.headers['X-IntegerEnumList'] != null) {
      b.headerIntegerEnumList.addAll(
        _i1
  ...
  factory InputAndOutputWithHeadersIo.fromResponse(
    ...
    if (response.headers['X-IntegerEnum'] != null) {
      b.headerIntegerEnum = IntegerEnum.values.byValue(
        int.parse(response.headers['X-IntegerEnum']!),
      );
    }
  ...
  String toString() {
    final helper =
        newBuiltValueToStringHelper('InputAndOutputWithHeadersIo')
          ...
          ..add('headerIntegerEnum', headerIntegerEnum)
          ..add('headerIntegerEnumList', headerIntegerEnumList);
...
abstract class InputAndOutputWithHeadersIoPayload
    with _i2.AWSEquatable<InputAndOutputWithHeadersIoPayload>
    implements
        Built<
          InputAndOutputWithHeadersIoPayload,
