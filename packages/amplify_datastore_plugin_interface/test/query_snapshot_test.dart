    ...
    group('with no query predicate or sort order', () {
      ...
      group('create event', () {
        test('returns a QuerySnapshot with one new item', () async {
          ...
          QuerySnapshot<Blog> updatedSnapshot = snapshot.withSubscriptionEvent(
    ...
    group('with a sort order', () {
      ...
      group('create event', () {
        test('returns a QuerySnapshot with one new item in the correct order',
          ...
          QuerySnapshot<Post> updatedSnapshot = snapshot.withSubscriptionEvent(
            event: subscriptionEvent,
          );
      ...
      group('update event', () {
        test(
          ...
          var expectedItems = [
            ...snapshot.items..removeWhere((item) => item.id == updatedPost.id),
            updatedPost,
          ]..sort((a, b) => a.rating.compareTo(b.rating));
