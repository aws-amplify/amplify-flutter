...
class QueryPredicateBuilder {
    companion object {
        @JvmStatic fun fromSerializedMap(serializedMap: Map<String, Any>?): QueryPredicate? {
            ...
            if (serializedMap.containsKey("queryPredicateGroup")) {
                ...
                // The first predicate in the list is either predicateOperation or
                // predicateGroup. We need
                // to know which one so that we can cast it appropriately and call the
                // `and` or `not` method
                if (predicates[0] is QueryPredicateOperation<*>) {
                    when (queryPredicateGroupMap["type"]) {
                        ...
                        "not" -> {
                            ...
                            // We don't have more query predicates for `not` operation type
                    ...
                    "not" -> {
                        ...
                        if (predicates.isNotEmpty()) {
                            throw IllegalArgumentException(
                                "More than one predicates added in the `not` " +
                                    "queryPredicate operation." + " Predicates Size: " +
                                    predicates.size
                            )
                        }
                        resultQueryPredicate =
