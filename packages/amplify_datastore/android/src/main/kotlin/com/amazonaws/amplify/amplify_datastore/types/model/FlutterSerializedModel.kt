...
data class FlutterSerializedModel(val serializedModel: SerializedModel) {
    ...
    private val serializedData: Map<String, Any> =
        parseSerializedDataMap(serializedModel.serializedData)

    // ignored fields
    private val id: String = serializedModel.id
    // ModelSchema -> SerializedModel should always have a name
    private val modelName: String = parseModelName(serializedModel.modelName)

    fun toMap(): Map<String, Any> {
        return mapOf("id" to id, "serializedData" to serializedData, "modelName" to modelName)
    }
    ...
    private fun parseSerializedDataMap(serializedData: Map<String, Any>): Map<String, Any> {
        ...
        return serializedData.mapValues {
            when (val value: Any = it.value) {
                ...
                // TODO add for other complex objects that can be returned or be part of the
                // codegen model
                // It seems we can ignore collection types for now as we aren't returning lists
                // of Models in hasMany relationships
                else -> value
            }
        }
    }
