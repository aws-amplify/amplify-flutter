...
@RunWith(RobolectricTestRunner::class) class AmplifyDataStorePluginTest {
    ...
    @Before fun setup() {
        ...
        amplifySuccessResults = mutableListOf<SerializedModel>(
            SerializedModel.builder()
                .serializedData(
                    mapOf(
                        "id" to "4281dfba-96c8-4a38-9a8e-35c7e893ea47", "title" to "Title 1",
                        "rating" to 4
                    )
                ).modelSchema(modelSchema).build(),
            SerializedModel.builder().serializedData(
                mapOf(
                    "id" to "43036c6b-8044-4309-bddc-262b6c686026", "title" to "Title 2",
        ...
        `when`(mockDataStore.getPlugin("awsDataStorePlugin"))
            .thenReturn(mockAmplifyDataStorePlugin)
    }

    @Test fun test_query_success_result() {
        doAnswer { invocation: InvocationOnMock ->
            ...
            .query(
                anyString(), any(QueryOptions::class.java), any<Consumer<Iterator<Model>>>(),
                any<Consumer<DataStoreException>>()
            )
        flutterPlugin.onQuery(
            mockResult,
            readMapFromFile(
                "query_api", "request/only_model_name.json",
                HashMap::class.java
            ) as HashMap<String, Any>
        )
        verify(mockResult, times(1)).success(
            readMapFromFile("query_api", "response/2_results.json", List::class.java)
        )
    }

    @Test fun test_query_with_predicates_success_zero_result() {
        val queryOptions = Where.matches(
            field("post.id").eq("123").or(
                field("rating").ge(4)
                    .and(not(field("created").eq("2020-02-20T20:20:20-08:00")))
            )
        )
    ...
    @Test fun test_query_api_error() {
        val testRequest: HashMap<String, Any> =
            (
                readMapFromFile("query_api", "request/only_model_name.json", HashMap::class.java)
                    as HashMap<String, Any>
                )
        ...
        doAnswer { invocation: InvocationOnMock ->
            assertEquals("Post", invocation.arguments[0])
            assertEquals(Where.matchesAll(), invocation.arguments[1])
            (invocation.arguments[3] as Consumer<DataStoreException>).accept(dataStoreException)
            null
        }.`when`(mockAmplifyDataStorePlugin)
            .query(
                anyString(), any(QueryOptions::class.java), any<Consumer<Iterator<Model>>>(),
    ...
    @Test fun test_query_malformed_request_casting_error() {
        ...
        verify(mockResult, times(1)).error(
            ...
            mapOf(
                ...
                "underlyingException" to "kotlin.TypeCastException: null cannot be cast " +
                    "to non-null type kotlin.String"
            )
        )
    }

    @Test fun test_delete_success_result_no_predicates() {
        val testRequest: HashMap<String, Any> =
            (
                readMapFromFile(
                    "delete_api", "request/instance_no_predicate.json",
                    HashMap::class.java
                ) as HashMap<String, Any>
                )
        ...
        val serializedModel = SerializedModel.builder().serializedData(serializedModelData)
            .modelSchema(modelSchema).build()

        val dataStoreItemChange =
            DataStoreItemChange.builder<SerializedModel>().item(serializedModel)
                .initiator(DataStoreItemChange.Initiator.LOCAL)
                .itemClass(SerializedModel::class.java)
                .type(DataStoreItemChange.Type.DELETE).build()
    ...
    @Test fun test_delete_api_error() {
        ...
        doAnswer { invocation: InvocationOnMock ->
            assertEquals(serializedModel, invocation.arguments[0])
            (invocation.arguments[2] as Consumer<DataStoreException>).accept(dataStoreException)
            null as Void?
        }.`when`(mockAmplifyDataStorePlugin).delete(
            any<SerializedModel>(),
            any<Consumer<DataStoreItemChange<SerializedModel>>>(),
    ...
    @Test fun test_delete_malformed_request_error() {
        val testRequest: HashMap<String, Any> = HashMap()
        ...
        verify(mockResult, times(1)).error(
            ...
            mapOf(
                "message" to ExceptionMessages.missingExceptionMessage,
                "recoverySuggestion" to ExceptionMessages.missingRecoverySuggestion,
                "underlyingException" to "kotlin.TypeCastException: null cannot be cast to " +
    ...
    @Test fun test_save_success_result_no_predicates() {
        ...
        val dataStoreItemChange =
            DataStoreItemChange.builder<SerializedModel>().item(serializedModel)
                .initiator(DataStoreItemChange.Initiator.LOCAL)
                .itemClass(SerializedModel::class.java)
                .type(DataStoreItemChange.Type.CREATE).build()
        ...
        doAnswer { invocation: InvocationOnMock ->
            ...
            any<SerializedModel>(), any<QueryPredicate>(),
            any<Consumer<DataStoreItemChange<SerializedModel>>>(),
            any<Consumer<DataStoreException>>()
        )
    ...
    @Test fun test_save_malformed_request_error() {
        val testRequest: HashMap<String, Any> = HashMap()
    ...
    @Test fun test_observe_success_event() {
        flutterPlugin = AmplifyDataStorePlugin(
        ...
        val eventData: HashMap<String, Any> =
            (
                readMapFromFile("observe_api", "post_type_success_event.json", HashMap::class.java)
                    as HashMap<String, Any>
                )
        val modelData = mapOf(
            "id" to "43036c6b-8044-4309-bddc-262b6c686026", "title" to "Title 2",
        ...
        val instance =
            SerializedModel.builder().serializedData(modelData).modelSchema(modelSchema).build()

        val dataStoreItemChange = DataStoreItemChange.builder<SerializedModel>().item(instance)
            .initiator(DataStoreItemChange.Initiator.LOCAL)
            .itemClass(SerializedModel::class.java).type(DataStoreItemChange.Type.CREATE)
            .build()
        ...
        doAnswer { invocation: InvocationOnMock ->
            ...
            any<Consumer<DataStoreException>>(), any<Action>()
        )
    ...
    @Test fun test_observe_error_event() {
        flutterPlugin = AmplifyDataStorePlugin(
            eventHandler = mockStreamHandler,
            hubEventHandler = mockHubHandler
        )
    ...
    @Test fun test_clear_success_result() {
        doAnswer { invocation: InvocationOnMock ->
    ...
    @Test fun test_clear_error() {
        ...
        doAnswer { invocation: InvocationOnMock ->
            (invocation.arguments[1] as Consumer<DataStoreException>).accept(dataStoreException)
            null as Void?
        }.`when`(mockAmplifyDataStorePlugin)
            .clear(any<Action>(), any<Consumer<DataStoreException>>())
    ...
    @Test fun test_nested_model_deserialization() {
        val nestedSerializedModelInput =
            mapOf<String, Any>(
                "id" to "af9cfa64-1ea9-46d6-b9e2-8203179d5392",
                "name" to "A brilliant Post", "rating" to 5,
        ...
        val nestedSerializedModelOutput =
            mapOf(
                "id" to "af9cfa64-1ea9-46d6-b9e2-8203179d5392", "name" to "A brilliant Post",
                "rating" to 5,
