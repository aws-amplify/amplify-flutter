...
class QueryPredicateBuilderTest {
    ...
    @Test fun test_when_rating_greater_or_equal() {
        Assert.assertEquals(
            rating.ge(4),
            QueryPredicateBuilder.fromSerializedMap(
                readMapFromFile(
                    "query_predicate", "rating_greater_or_equal.json",
                    HashMap::class.java
                ) as HashMap<String, Any>
            )
        )
    ...
    @Test fun test_complex_nested_and_or() {
        Assert.assertEquals(
            rating.le(4).and(id.contains("abc").or(title.beginsWith("def"))),
            QueryPredicateBuilder.fromSerializedMap(
                readMapFromFile(
                    "query_predicate", "complex_nested.json",
                    HashMap::class.java
                ) as HashMap<String, Any>
            )
        )
    ...
    @Test fun test_when_group_with_only_and() {
        Assert.assertEquals(
            rating.between(1, 4).and(id.contains("abc")).and(title.beginsWith("def"))
                .and(created.eq("2020-02-20T20:20:20-08:00")),
            QueryPredicateBuilder.fromSerializedMap(
                readMapFromFile(
                    "query_predicate", "group_with_only_and.json",
                    HashMap::class.java
                ) as HashMap<String, Any>
            )
        )
    ...
    @Test fun test_when_mixed_and_or() {
        Assert.assertEquals(
            rating.lt(4).and(id.contains("abc")).or(title.contains("def")),
            QueryPredicateBuilder.fromSerializedMap(
                readMapFromFile(
                    "query_predicate", "group_mixed_and_or.json",
                    HashMap::class.java
                ) as HashMap<String, Any>
            )
        )
    ...
    @Test fun test_when_rating_gt_but_not_eq() {
        Assert.assertEquals(
            rating.gt(4).and(not(rating.eq(1))),
            QueryPredicateBuilder.fromSerializedMap(
                readMapFromFile(
                    "query_predicate", "mixed_with_not.json",
                    HashMap::class.java
                ) as HashMap<String, Any>
            )
        )
    ...
    @Test fun test_when_negate_complex_predicate() {
        Assert.assertEquals(
            QueryPredicateGroup.not(rating.eq(1).and(rating.eq(4).or(title.contains("crap")))),
            QueryPredicateBuilder.fromSerializedMap(
                readMapFromFile(
                    "query_predicate", "negate_complex_predicate.json",
                    HashMap::class.java
                ) as HashMap<String, Any>
            )
        )
