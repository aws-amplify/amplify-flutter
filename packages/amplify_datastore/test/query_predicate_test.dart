...
void main() {
  ...
  group('query predicate serialization', () {
    ...
    test('bad model id field naming backwards compatibility', () async {
      final QueryPredicate testPredicateWithBadIdFiledNaming =
          const QueryField<String>(fieldName: 'blog.id').ne('123');
      expect(
    ...
    test('when rating less or equal AND (id contains or title begins with)',
        () async {
      final QueryPredicate testPredicate = Post.RATING
          .le(4)
    ...
    test('when rating gt but not eq', () async {
      final QueryPredicate testPredicate =
    ...
    test('when negate complex predicate', () async {
      final QueryPredicate testPredicate = not(
        Post.RATING
            .eq(1)
    ...
    test('when value is a temporal type', () async {
      final QueryPredicate testPredicate = Post.CREATED.eq(
  ...
  group('query predicate comparison', () {
    ...
    final stringListTypeModel1 = StringListTypeModel(
      value: ['abc'],
    );

    final stringListTypeModel2 = StringListTypeModel(
      value: ['abc', 'xyz'],
    );

    final stringListTypeModel3 = StringListTypeModel(
      value: ['xyz'],
    );

    test('equals', () {
      final QueryPredicate testPredicate = Post.LIKECOUNT.eq(1);
    ...
    test('equals (ID)', () {
      final QueryPredicate testPredicate = Post.ID.eq(post2.id);
    ...
    test('equals (ModelIdentifier)', () {
      final QueryPredicate testPredicate = Post.MODEL_IDENTIFIER.eq(
    ...
    test('not equals', () {
      final QueryPredicate testPredicate = Post.LIKECOUNT.ne(1);
      expect(testPredicate.evaluate(post1), isFalse);
      expect(testPredicate.evaluate(post2), isTrue);
    ...
    test('less than', () {
      final QueryPredicate testPredicate = Post.LIKECOUNT.lt(5);
      expect(testPredicate.evaluate(post1), isTrue);
      expect(testPredicate.evaluate(post2), isFalse);
      expect(testPredicate.evaluate(post4), isFalse);
    });
    ...
    test('less than or equal', () {
      final QueryPredicate testPredicate = Post.LIKECOUNT.le(10);
      expect(testPredicate.evaluate(post1), isTrue);
      expect(testPredicate.evaluate(post2), isTrue);
    ...
    test('greater than', () {
      final QueryPredicate testPredicate = Post.LIKECOUNT.gt(5);
      expect(testPredicate.evaluate(post1), isFalse);
      expect(testPredicate.evaluate(post2), isTrue);
      expect(testPredicate.evaluate(post4), isFalse);
    });
    ...
    test('greater than or equal', () {
      final QueryPredicate testPredicate = Post.LIKECOUNT.ge(10);
      expect(testPredicate.evaluate(post1), isFalse);
      expect(testPredicate.evaluate(post2), isTrue);
    ...
    test('between', () {
      final QueryPredicate testPredicate = Post.LIKECOUNT.between(5, 100);
      expect(testPredicate.evaluate(post1), isFalse);
      expect(testPredicate.evaluate(post2), isTrue);
      expect(testPredicate.evaluate(post3), isTrue);
      expect(testPredicate.evaluate(post4), isFalse);
    ...
    test('contains', () {
      final QueryPredicate testPredicate = Post.TITLE.contains('one');
      expect(testPredicate.evaluate(post1), isTrue);
      expect(testPredicate.evaluate(post2), isFalse);
    });

    test('beginsWith', () {
      final QueryPredicate testPredicate = Post.TITLE.beginsWith('post o');
      expect(testPredicate.evaluate(post1), isTrue);
      expect(testPredicate.evaluate(post2), isFalse);
    });
    ...
    test('not', () {
      final QueryPredicate testPredicate = not(Post.RATING.lt(5));
      expect(testPredicate.evaluate(post1), isFalse);
      expect(testPredicate.evaluate(post2), isTrue);
    });

    test('attributeExists', () {
      final QueryPredicate testPredicate = Post.LIKECOUNT.attributeExists();
      expect(testPredicate.evaluate(post4), isFalse);
      expect(testPredicate.evaluate(post2), isTrue);
    });

    test('Temporal type', () {
      final QueryPredicate testPredicate = Post.CREATED.lt(
        TemporalDateTime(
    ...
    group('List type', () {
      test('contains', () {
        final QueryPredicate testPredicate = StringListTypeModel.VALUE.contains(
          'abc',
        );
