...
void main() {
  group('throttleByCountAndTime', () {
    test('should throttle by count if only count is provided', () {
      var stream = Stream.fromIterable(List.generate(10, (index) => index));

      var throttledStream = stream.throttleByCountAndTime(throttleCount: 2);
    ...
    test('should not throttle if count is 1', () {
      ...
      var throttledStream = stream.throttleByCountAndTime(throttleCount: 1);
    ...
    group('source stream types', () {
      ...
      test(
        'multiple listeners all get values if the source is a broadcast',
        () {
          var values = [];
          var values2 = [];
