  ...
  group('PackageInfo', () {
    test('compatibleWithActiveSdk', () async {
      ...
      final repo = await d.repo([
        d.package('stable_pkg'),
        d.package(
          'preview_pkg',
          sdkConstraint: '^3.2.0-0',
        ),
      ]).create();
      final stablePackage = repo['stable_pkg'];
      final previewPackage = repo['preview_pkg'];
      expect(
        repo.compatibleWithActiveSdk(stablePackage),
        isTrue,
      );
      expect(
        repo.compatibleWithActiveSdk(previewPackage),
