...
class Repo {
  ...
  late final Directory workingDirectory =
      fileSystem.directory(aftConfig.workingDirectory);
  ...
  bool compatibleWithActiveSdk(PackageInfo package) =>
      package.dartSdkConstraint.allows(activeDartSdkVersion);

  /// The version of the current environment's Dart SDK.
  ///
  /// This parses the version from calling `dart --version`.
  late final Version activeDartSdkVersion = () {
    final ProcessResult(
      :exitCode,
      :stdout,
      :stderr,
    ) = processManager.runSync(<String>['dart', '--version']);
    if (exitCode != 0) {
      throw Exception(
  ...
  static final _versionRegex = RegExp(r'\d+\.\d+\.\d+(-[a-zA-Z\d]+)?');
  ...
  Future<List<PackageInfo>> writeChanges({
    ...
    for (final package in packages ?? allPackages.values.toList()) {
      ...
      if (changelogUpdate != null && changelogUpdate.hasUpdate) {
        await fileSystem
            .file(p.join(package.path, 'CHANGELOG.md'))
  ...
  Version bumpVersion(
    ...
    changelogUpdates[package] = package.changelog(fileSystem).update(
      commits: {
    ...
    if (summaryPackage != null && includeInChangelog) {
      ...
      changelogUpdates[summaryPackage] =
          summaryPackage.changelog(fileSystem).update(
        commits: {
          ...?currentChangelogUpdate?.commits,
          commit,
        },
