...
abstract class StorageClassAnalysis
  ...
  /// Specifies data related to access patterns to be collected and made available to analyze the tradeoffs between different storage classes for an Amazon S3 bucket.
  factory StorageClassAnalysis({StorageClassAnalysisDataExport? dataExport}) {
  ...
  static const List<_i2.SmithySerializer<StorageClassAnalysis>> serializers = [
    StorageClassAnalysisRestXmlSerializer()
  ];

  /// Specifies how data related to the storage class analysis for an Amazon S3 bucket should be exported.
  StorageClassAnalysisDataExport? get dataExport;
  @override
...
class StorageClassAnalysisRestXmlSerializer
    extends _i2.StructuredSmithySerializer<StorageClassAnalysis> {
      ...
      switch (key) {
        case 'DataExport':
          result.dataExport.replace((serializers.deserialize(
            value,
            specifiedType: const FullType(StorageClassAnalysisDataExport),
          ) as StorageClassAnalysisDataExport));
      }
    }
  ...
  Iterable<Object?> serialize(
    ...
    if (dataExport != null) {
      result$
        ..add(const _i2.XmlElementName('DataExport'))
