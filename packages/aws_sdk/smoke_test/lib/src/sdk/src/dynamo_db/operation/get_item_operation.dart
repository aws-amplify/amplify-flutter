...
class GetItemOperation extends _i1
    .HttpOperation<GetItemInput, GetItemInput, GetItemOutput, GetItemOutput> {
  /// The `GetItem` operation returns a set of attributes for the item with the given primary key. If there is no matching item, `GetItem` does not return any data and there will be no `Item` element in the response.
  ///
  /// `GetItem` provides an eventually consistent read by default. If your application requires a strongly consistent read, set `ConsistentRead` to `true`. Although a strongly consistent read might take more time than an eventually consistent read, it always returns the last updated value.
  GetItemOperation({
  ...
  @override
  late final List<
      _i1.HttpProtocol<GetItemInput, GetItemInput, GetItemOutput,
          GetItemOutput>> protocols = [
  ...
  @override
  _i1.HttpRequest buildRequest(GetItemInput input) => _i1.HttpRequest((b) {
        b.method = 'POST';
        b.path = r'/';
      });
  @override
  int successCode([GetItemOutput? output]) => 200;
  @override
  GetItemOutput buildOutput(
    GetItemOutput payload,
    _i4.AWSBaseHttpResponse response,
  ) =>
      GetItemOutput.fromResponse(
        payload,
        response,
      );
  @override
  ...
  @override
  _i1.SmithyOperation<GetItemOutput> run(
    GetItemInput input, {
