...
class BatchGetItemOperation extends _i1.HttpOperation<BatchGetItemInput,
    BatchGetItemInput, BatchGetItemOutput, BatchGetItemOutput> {
  /// The `BatchGetItem` operation returns the attributes of one or more items from one or more tables. You identify requested items by primary key.
  ///
  /// A single operation can retrieve up to 16 MB of data, which can contain as many as 100 items. `BatchGetItem` returns a partial result if the response size limit is exceeded, the table's provisioned throughput is exceeded, more than 1MB per partition is requested, or an internal processing failure occurs. If a partial result is returned, the operation returns a value for `UnprocessedKeys`. You can use this value to retry the operation starting with the next item to get.
  ///
  ...
  @override
  late final List<
      _i1.HttpProtocol<BatchGetItemInput, BatchGetItemInput, BatchGetItemOutput,
          BatchGetItemOutput>> protocols = [
  ...
  @override
  _i1.HttpRequest buildRequest(BatchGetItemInput input) => _i1.HttpRequest((b) {
        b.method = 'POST';
        b.path = r'/';
      });
  @override
  int successCode([BatchGetItemOutput? output]) => 200;
  @override
  BatchGetItemOutput buildOutput(
    BatchGetItemOutput payload,
    _i4.AWSBaseHttpResponse response,
  ) =>
      BatchGetItemOutput.fromResponse(
        payload,
        response,
      );
  @override
  ...
  @override
  _i1.SmithyOperation<BatchGetItemOutput> run(
