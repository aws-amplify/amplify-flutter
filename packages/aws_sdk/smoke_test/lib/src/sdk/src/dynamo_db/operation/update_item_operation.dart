...
class UpdateItemOperation extends _i1.HttpOperation<UpdateItemInput,
    UpdateItemInput, UpdateItemOutput, UpdateItemOutput> {
  /// Edits an existing item's attributes, or adds a new item to the table if it does not already exist. You can put, delete, or add attribute values. You can also perform a conditional update on an existing item (insert a new attribute name-value pair if it doesn't exist, or replace an existing name-value pair if it has certain expected attribute values).
  ///
  /// You can also return the item's attribute values in the same `UpdateItem` operation using the `ReturnValues` parameter.
  UpdateItemOperation({
  ...
  @override
  late final List<
      _i1.HttpProtocol<UpdateItemInput, UpdateItemInput, UpdateItemOutput,
          UpdateItemOutput>> protocols = [
  ...
  @override
  _i1.HttpRequest buildRequest(UpdateItemInput input) => _i1.HttpRequest((b) {
        b.method = 'POST';
        b.path = r'/';
      });
  @override
  int successCode([UpdateItemOutput? output]) => 200;
  @override
  UpdateItemOutput buildOutput(
    UpdateItemOutput payload,
    _i4.AWSBaseHttpResponse response,
  ) =>
      UpdateItemOutput.fromResponse(
        payload,
        response,
      );
  @override
  ...
  @override
  _i1.SmithyOperation<UpdateItemOutput> run(
