...
/// {@template amplify_firehose_dart.sqlite_record_storage}
...
final class SqliteRecordStorage extends RecordStorage {
  /// {@macro amplify_firehose_dart.sqlite_record_storage}
  ///
  ...
  /// {@macro amplify_firehose_dart.sqlite_record_storage}
  ///
  /// Opens the database and eagerly queries the initial cache size.
  ...
  Future<Map<String, List<Record>>> doGetRecordsByStream() async {
    // Firehose has no partition key, so no PARTITION BY — just global
    // ORDER BY stream_name, id with limits on count and size.
    final results = await _db.customSelect(
      ...
      variables: [
        ...
        Variable.withInt(maxBatchSizeBytes),
      ],
