...
/// {@template amplify_firehose_dart.indexeddb_record_storage}
...
final class IndexedDbRecordStorage extends RecordStorage {
  ...
  /// {@macro amplify_firehose_dart.indexeddb_record_storage}
  ///
  ...
  static Future<IndexedDbRecordStorage> create({
    ...
    final database = await _openDatabase('amplify_firehose_$identifier');
  ...
  static const _storeName = 'firehose_records';
  ...
  static Future<IDBDatabase> _openDatabase(String dbName) async {
    ...
    void onUpgradeNeeded(IDBVersionChangeEvent event) {
      final database =
          event.target?.getProperty<IDBDatabase>('result'.toJS);
  ...
  static Future<bool> checkIsSupported() async {
    ...
    try {
      final request = indexedDB!.open('firehose_idb_test', 1);
      await request.future;
      return true;
    } on Object {
      return false;
