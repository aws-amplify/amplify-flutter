...
/// Lightweight Cognito auth helper for integration tests.
///
/// Makes direct HTTP calls to Cognito APIs without requiring
/// amplify_auth_cognito as a dependency. This keeps the kinesis
/// example package decoupled from the Amplify auth stack.
library;
...
class CognitoAuthHelper {
  ...
  Future<TemporaryCredentials> authenticate() async {
  ...
  Future<TemporaryCredentials> _getCredentials(String idToken) async {
    final logins = {
  ...
  ) => _postJson(_cognitoEndpoint, target, body);

  Future<Map<String, dynamic>> _identityRequest(
    String target,
    Map<String, dynamic> body,
  ) => _postJson(_identityEndpoint, target, body);
