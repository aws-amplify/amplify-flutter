...
void main() {
  group('StorageS3Service', () {
    ...
    group('list() API', () {
      ...
      test(
        ...
        () async {
          ...
          expect(request.prefix, TestPathResolver.path);
              ...
              PaginatedResult<ListObjectsV2Output, int, String>(
                ListObjectsV2Output(
                  ...
                  commonPrefixes: [
                    ...
                    CommonPrefix(prefix: testSubPaths[2]),
                  ],
                  delimiter: testDelimiter,
                  name: testBucketName,
                  maxKeys: testPageSize,
      ...
      test(
        ...
        ' with status code 403 returned from service',
        () async {
          const testOptions = StorageListOptions();
      ...
      test(
        ...
        () async {
          ...
          final smithyOperation1 =
              MockSmithyOperation<
                PaginatedResult<ListObjectsV2Output, int, String>
              >();
          final smithyOperation2 =
              MockSmithyOperation<
                PaginatedResult<ListObjectsV2Output, int, String>
              >();
      ...
      test('should handle AWSHttpException and throw NetworkException', () {
        ...
        when(() => s3Client.listObjectsV2(any())).thenThrow(testException);
    ...
    group('getProperties() API', () {
      ...
      const testMetadata = {'filename': 'hello.jpg', 'uploader': '123'};
      const testSize = 1024;
      ...
      test(
        'should invoke S3Client.headObject with expected parameters',
        () async {
          const testOptions = StorageGetPropertiesOptions(
      ...
      test(
        ...
        ' with status code 404 returned from service',
        () async {
          const testOptions = StorageGetPropertiesOptions();
          const testUnknownException = UnknownSmithyHttpException(
            statusCode: 404,
      ...
      test('should handle AWSHttpException and throw NetworkException', () {
        ...
        when(() => s3Client.headObject(any())).thenThrow(testException);
    ...
    group('getUrl() API', () {
      ...
      setUpAll(() {
        registerFallbackValue(AWSHttpRequest.get(testUrl));
        registerFallbackValue(
      ...
      test(
        'should invoke AWSSigV4Signer.presign with correct parameters',
        () async {
          // Must await, or the assertions inside never run.
          await runZoned(() async {
            ...
            // Select by type, not position: capture order isn't stable across
            // compilers.
            ...
            final requestParam = capturedParams
                .whereType<AWSHttpRequest>()
                .last;
            expect(
            ...
            final configParam = capturedParams
                .whereType<S3ServiceConfiguration>()
                .last;
            expect(configParam.signBody, false);
            expect(configParam.chunked, false);

            expect(capturedParams.whereType<Duration>().last, testExpiresIn);
      ...
      test(
        'should create a new signer scope on every call of getUrl',
        () async {
          const testOptions = StorageGetUrlOptions();
      ...
      test(
        ...
        () async {
          const testOptions = StorageGetUrlOptions(
            pluginOptions: S3GetUrlPluginOptions(validateObjectExistence: true),
          );
          const testUnknownException = UnknownSmithyHttpException(
            statusCode: 404,
            body: 'Nod found.',
          ...
          await expectLater(
            storageS3Service.getUrl(path: testPath, options: testOptions),
            throwsA(isA<StorageNotFoundException>()),
          );
          ...
          expect(capturedRequest.key, TestPathResolver.path);
        },
      );

      test(
        'should invoke s3Client.headObject when validateObjectExistence option is'
        ' set to true and specified targetIdentityId',
        () async {
          const testOptions = StorageGetUrlOptions(
            pluginOptions: S3GetUrlPluginOptions(validateObjectExistence: true),
          );
      ...
      group('bucket name has dots (".")', () {
        ...
        test('generate path style URL', () async {
          const testOptions = StorageGetUrlOptions();
    ...
    group('copy() API', () {
      ...
      test(
        'should invoke S3Client.copyObject with expected parameters',
        () async {
      ...
      test(
        'should handle AWSHttpException and throw NetworkException',
        () async {
          const testOptions = StorageCopyOptions();
          final testException = AWSHttpException(
          ...
          when(() => s3Client.copyObject(any())).thenThrow(testException);
    ...
    group('remove() API', () {
      ...
      setUpAll(() {
        registerFallbackValue(
          DeleteObjectRequest(bucket: 'fake bucket', key: 'dummy key'),
        );
      });

      test(
        'should invoke S3Client.deleteObject with default access level',
        () async {
      ...
      test(
        'should invoke S3Client.deleteObject with expected parameters',
        () async {
          const testOptions = StorageRemoveOptions();
          final testDeleteObjectOutput = DeleteObjectOutput();
          final smithyOperation = MockSmithyOperation<DeleteObjectOutput>();
      ...
      test(
        ...
        () async {
          ...
          when(() => s3Client.deleteObject(any())).thenThrow(testException);
    ...
    group('removeMany() API', () {
      ...
      setUpAll(() async {
        resolvedPaths = await pathResolver.resolvePaths(paths: testPaths);
        registerFallbackValue(
      ...
      test(
        'should invoke S3Client.deleteObjects with default access level',
        () async {
      ...
      test(
        'should invoke S3Client.deleteObjects with expected parameters',
        () async {
          const testOptions = StorageRemoveManyOptions();
          final testDeleteObjectsOutput1 = DeleteObjectsOutput(
            ...
            errors: List.generate(
              ...
              (index) =>
                  Error(key: resolvedPaths[index], message: 'some error'),
            ),
          );
      ...
      test(
        ...
        () async {
          ...
          expect(
            storageS3Service.removeMany(paths: testPaths, options: testOptions),
            throwsA(isA<StorageAccessDeniedException>()),
          );
        },
      );
      ...
      test('should handle AWSHttpRequest and throw NetworkException', () async {
        ...
        when(() => s3Client.deleteObjects(any())).thenThrow(testException);
