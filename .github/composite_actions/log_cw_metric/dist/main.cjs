...
(function dartProgram() {
  ...
  A = {JS_CONST: function JS_CONST() {
    ...
    main(args) {
      return A.wrapMain(A.log_cw_metric__logMetric$closure());
    },
    logMetric() {
      var $async$goto = 0,
        $async$completer = A._makeAsyncAwaitCompleter(type$.void),
        metricName, testType, workingDirectory, category, t4, t5, framework, flutterDartChannel, dartVersion, flutterVersion, dartCompiler, platform, platformVersion, value, dimensions, cloudArgs, t1, jobStatus, inputValue, matrixRawInput, matrixValues, t2, t3, jobIdentifier;
        ...
        for (;;)
          switch ($async$goto) {
            case 0:
              // Function start
              t1 = init.G;
              jobStatus = A.Core_getRequiredInput(A._asJSObject(t1.core), "job-status");
              inputValue = A._asString(A._asJSObject(t1.core).getInput("matrix"));
              matrixRawInput = inputValue.length === 0 ? "" : inputValue;
              matrixValues = new A.logMetric_parseMatrixInput().call$1(matrixRawInput);
              t2 = A._asString(A._asJSObject(A._asJSObject(t1.github).context).job);
              t3 = matrixValues == null || matrixValues.length === 0 ? "" : "(" + matrixValues + ")";
              jobIdentifier = B.JSString_methods.trim$0(t2 + " " + t3);
              A._asJSObject(t1.core).info("Job identifier: " + jobIdentifier);
              metricName = A.Core_getRequiredInput(A._asJSObject(t1.core), "metric-name");
              testType = A.Core_getRequiredInput(A._asJSObject(t1.core), "test-type");
              t2 = type$.JSArray_String;
              if (!B.JSArray_methods.contains$1(A._setArrayType(["canary", "e2e", "unit"], t2), testType))
              ...
              t3 = type$.String;
              category = A.IterableExtension_firstWhereOrNull(A._setArrayType(["canaries", "analytics", "api", "auth", "authenticator", "core", "datastore", "db_common", "push", "secure_storage", "storage", "aws_common", "aws_signature_v4", "smithy", "worker_bee", "amplify_flutter", "amplify_foundation_dart_bridge", "amplify_foundation_dart", "amplify_lints", "amplify_native_legacy_wrapper", "kinesis", "connect_client", "event_enrichment", "pub_server"], t2), B.JSString_methods.get$contains(workingDirectory), t3);
              if (category == null)
              ...
              t5 = A._asString(A._asJSObject(A._asJSObject(t1.github).context).job);
              inputValue = A._asString(A._asJSObject(t1.core).getInput("framework"));
              framework = inputValue.length === 0 ? "none" : inputValue;
              if (!B.JSArray_methods.contains$1(A._setArrayType(["dart", "flutter"], t2), framework))
                throw A.wrapException(A.Exception_Exception("framework input of " + framework + " must be one of: dart, flutter"));
              inputValue = A._asString(A._asJSObject(t1.core).getInput("flutter-dart-channel"));
              flutterDartChannel = inputValue.length === 0 ? "none" : inputValue;
              inputValue = A._asString(A._asJSObject(t1.core).getInput("dart-version"));
              dartVersion = inputValue.length === 0 ? "none" : inputValue;
              inputValue = A._asString(A._asJSObject(t1.core).getInput("flutter-version"));
              flutterVersion = inputValue.length === 0 ? "none" : inputValue;
              inputValue = A._asString(A._asJSObject(t1.core).getInput("dart-compiler"));
              dartCompiler = inputValue.length === 0 ? "none" : inputValue;
              inputValue = A._asString(A._asJSObject(t1.core).getInput("platform"));
              platform = inputValue.length === 0 ? "none" : inputValue;
              inputValue = A._asString(A._asJSObject(t1.core).getInput("platform-version"));
              platformVersion = inputValue.length === 0 ? "none" : inputValue;
              value = jobStatus === "failure" ? "1" : "0";
              dimensions = A.LinkedHashMap_LinkedHashMap$_literal(["test-type", testType, "category", category, "workflow-name", t4 + "/" + t5, "framework", framework, "flutter-dart-channel", flutterDartChannel, "dart-version", dartVersion, "flutter-version", flutterVersion, "dart-compiler", dartCompiler, "platform", platform, "platform-version", platformVersion, "event-name", A._asString(A._asJSObject(A._asJSObject(t1.github).context).eventName)], t3, t3);
              t4 = A._instanceType(dimensions)._eval$1("LinkedHashMapEntriesIterable<1,2>");
              cloudArgs = A._setArrayType(["cloudwatch", "put-metric-data", "--metric-name", metricName, "--namespace", "GithubCanaryApps", "--value", value, "--dimension", A.MappedIterable_MappedIterable(new A.LinkedHashMapEntriesIterable(dimensions, t4), t4._eval$1("String(Iterable.E)")._as(new A.logMetric_closure()), t4._eval$1("Iterable.E"), t3).join$1(0, ",")], t2);
              t3 = $.$get$processManager();
              t2 = A._setArrayType(["aws"], t2);
              B.JSArray_methods.addAll$1(t2, cloudArgs);
              $async$goto = 2;
              return A._asyncAwait(t3.run$1(t2), $async$logMetric);
            case 2:
              // returning from await.
              A._asJSObject(t1.core).info("Sent cloudwatch metric with args: " + A.S(cloudArgs));
              // implicit return
              return A._asyncReturn(null, $async$completer);
          }
      });
    ...
    logMetric_parseMatrixInput: function logMetric_parseMatrixInput() {
    },
    logMetric_parseMatrixInput_closure: function logMetric_parseMatrixInput_closure() {
    },
    logMetric_closure: function logMetric_closure() {
    },
    printString(string) {
    ...
    Core_getRequiredInput(_this, $name) {
      var inputValue = A._asString(_this.getInput($name));
      return inputValue.length === 0 ? A.throwExpression(A.StateError$('Input "' + $name + '" was required but no value was passed')) : inputValue;
    },
  ...
  A.NodeProcessManager.prototype = {
    run$1(command) {
      return this.run$body$NodeProcessManager(type$.List_Object._as(command));
    },
    run$body$NodeProcessManager(command) {
      var $async$goto = 0,
        ...
        for (;;)
          switch ($async$goto) {
            case 0:
              // Function start
              $async$goto = 3;
              return A._asyncAwait($async$self.start$7$environment$includeParentEnvironment$mode$pipe$runInShell$workingDirectory(command, null, true, B.ProcessStartMode_0, null, false, null), $async$run$1);
            case 3:
              // returning from await.
              ...
              stdoutSub = t1.$ti._eval$1("StreamTransformer<Stream.T,String>")._as(B.C_LineSplitter).bind$1(t1).listen$1(new A.NodeProcessManager_run_closure(false, stdout));
              t1 = process._stderr;
              t2 = A._instanceType(t1)._eval$1("_ControllerStream<1>");
              t1 = t2._eval$1("StreamTransformer<Stream.T,String>")._as(B.Utf8Decoder_true).bind$1(new A._ControllerStream(t1, t2));
              stderrSub = t1.$ti._eval$1("StreamTransformer<Stream.T,String>")._as(B.C_LineSplitter).bind$1(t1).listen$1(new A.NodeProcessManager_run_closure0(false, stderr));
              $async$handler = 4;
              $async$goto = 7;
              return A._asyncAwait(process.get$exitCode(), $async$run$1);
            case 7:
              // returning from await.
              exitCode = $async$result;
              stdout.toString;
              stderr.toString;
              $async$returnValue = new A.ProcessResult();
              $async$next = [1];
              // goto finally
              $async$goto = 5;
              break;
  ...
  A.logMetric_parseMatrixInput.prototype = {
    call$1(matrixRawInput) {
      ...
      if (matrixRawInput.length === 0 || matrixRawInput === "{}")
        return null;
      A._asJSObject(init.G.core).info("Accepted matrix input: <" + matrixRawInput + ">");
      t1 = A.stringReplaceAllUnchecked(matrixRawInput, "\n", "");
      t1 = A.stringReplaceAllUnchecked(t1, "\\", "");
      t1 = type$.Map_String_dynamic._as(B.C_JsonCodec.decode$1(A.stringReplaceAllUnchecked(t1, " ", ""))).get$values();
      t2 = A._instanceType(t1);
      return A.MappedIterable_MappedIterable(t1, t2._eval$1("@(Iterable.E)")._as(new A.logMetric_parseMatrixInput_closure()), t2._eval$1("Iterable.E"), type$.dynamic).join$1(0, ", ");
    },
    $signature: 58
  };
  A.logMetric_parseMatrixInput_closure.prototype = {
    call$1(e) {
      return e;
    },
    $signature: 12
  };
  A.logMetric_closure.prototype = {
    call$1(e) {
  ...
  (function installTearOffs() {
    ...
    _instance(J.JSString.prototype, "get$contains", 1, 1, null, ["call$2", "call$1"], ["contains$2", "contains$1"], 60, 0, 0);
    ...
    _static(A, "async___rootScheduleMicrotask$closure", 4, null, ["call$4"], ["_rootScheduleMicrotask"], 66, 0);
    _static(A, "async___rootCreateTimer$closure", 5, null, ["call$5"], ["_rootCreateTimer"], 67, 0);
    _static(A, "async___rootCreatePeriodicTimer$closure", 5, null, ["call$5"], ["_rootCreatePeriodicTimer"], 68, 0);
    _static(A, "async___rootPrint$closure", 4, null, ["call$4"], ["_rootPrint"], 69, 0);
    _static(A, "async___rootFork$closure", 5, null, ["call$5"], ["_rootFork"], 70, 0);
    ...
    _instance_1_u(_ = A._StreamSinkImpl.prototype, "get$_completeDoneValue", "_completeDoneValue$1", 14);
    _instance_2_u(_, "get$_completeDoneError", "_completeDoneError$2", 54);
    ...
    _instance(_, "get$_registerUnaryCallback", 0, 4, null, ["call$2$4", "call$4"], ["_registerUnaryCallback$2$4", "_registerUnaryCallback$4"], 23, 0, 0);
    _instance(_, "get$_registerBinaryCallback", 0, 4, null, ["call$3$4", "call$4"], ["_registerBinaryCallback$3$4", "_registerBinaryCallback$4"], 24, 0, 0);
    ...
    _static_0(A, "log_cw_metric__logMetric$closure", "logMetric", 3);
